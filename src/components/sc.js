...
const variablesCollection = new WeakMap();
...
export default class ReactJsonSchema {
  ...
  createComponent(schema) {
    ...
    let { component, children, dynamicVar, text, ...rest } = schema;
    ...
    this.replaceString(rest);
    return createElement(Component, rest, Children );
  }

  replaceString(_schema) {
    const schema = _schema;
    const handlerMap = this.getVariablesMap();
    const handlers = [
      'dynamicVar1', 'dynamicVar2', 'dynamicVar3',"dynamicVar4","dynamicVar5","dynamicVar6","dynamicVar7","dynamicVar8","dynamicVar9","dynamicVar10" 
    ];
  ...
  getVariablesMap() {
      return variablesCollection.get(this);
  }
  setVariablesMap(variablesMap) {
      variablesCollection.set(this, variablesMap);
  }
}
